...
!function($) {
	...
	var Notify = function(el, options){
		...
		//不同类型设置不同的提示音
		if($fags){
			$this.notifyset({sound: './audio/notify.wav'});
		}else{
			$this.notifyset({sound: './audio/notify.wav'});
		}
		...
		//console.log('提示音类型:'+this.opts.type);
		//console.log('是否是手机端:'+$fags);
		//console.log('声音来源:'+settings.sound);
		...
		if(this.opts.sound && audio && audio.play()){
			//console.log('播放正确');
		}else{
			console.log('播放错误');
		}
	...
	Notify.prototype.notifyset = function(options){
